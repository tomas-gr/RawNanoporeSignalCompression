...
decompress_cli_args parse_args(int argc, char **argv, char *curr_dir)
{
	decompress_cli_args res;
...
int main(int argc, char **argv)
{
	...
	const Pod5WriterOptions_t writer_options = {0, VBZ_SIGNAL_COMPRESSION, 0, 0};
	...
	writer = pod5_create_file(args.out_filename.data(), "PGNano decompressor", &writer_options);
    ...
    pgnano::standalone::Decompressor decompressor(args.bam_filename, args.levels_table_filename);
	...
	for (size_t current_batch_idx = 0; current_batch_idx < batch_count; current_batch_idx++)
	{
        ...
        int16_t *decompressed_signal[batch_row_count];
        uint32_t decompressed_signal_words[batch_row_count];//FIXME: returned value should be uint32_t and not a size_t converted to uint32_t
			 ...
			 current_batch_row++)
		{
            ...
            decompressed_signal_words[current_batch_row] = decompressor.retrieve_sample_count(original_signal[current_batch_row]);
            decompressed_signal[current_batch_row] = (int16_t *) malloc(decompressed_signal_words[current_batch_row] * sizeof(int16_t));//FIXME: Incorrect memory alloc
            decompressor.decompress(from_pod5_id(read_record_batch_array[current_batch_row].read_id), original_signal[current_batch_row], decompressed_signal[current_batch_row]);
		...
		LOG_PROGRAM_ERROR(pod5_add_reads_data(// FIXME: incorrect signal array passed
			...
			const_cast<const int16_t **>(decompressed_signal), decompressed_signal_words));
