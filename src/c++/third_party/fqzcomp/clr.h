...
class RangeCoder {
   ...
   public:
    uc *in_buf;//FIXME: ask!: Can this variable be uc const *?
    uc *out_buf;

    void input(char *in) { out_buf = in_buf = (uc *)in; }
    void input(uc *in) { out_buf = in_buf = (uc *)in; }//FIXME: ASK!

    void output(char *out) { in_buf = out_buf = (uc *)out; }
    void output(uc *out) { in_buf = out_buf = (uc *)out; }//FIXME: ASK!
    ...
    constexpr size_t FinishEncode(void) {
        DO(8)
    ...
    size_t Encode(uint32_t cumFreq, uint32_t freq, uint32_t totFreq) {
        ...
        while (range < TOP) {
            ...
            if (uc((low ^ (low + range)) >> 56))
                range = ((uint32_t(low) | (TOP - 1)) - uint32_t(low));// TODO: ask! => (TOP - 1) & (~(uint32_t(low)))
            *out_buf++ = low >> 56, range <<= 8, low <<= 8;//At most 8 iterations. => No more than 8 bytes extra buffer to avoid overflow TODO: ask!
            i++;
        }
        return i;
    }
