...
template <typename Int16T, bool UseDelta, bool UseZigzag>
buf_sizes_N03 encode_N03(Int16T const * in,
              uint8_t * SVB_RESTRICT keys,
...
//     // }
// #endif
    return encode_scalar_N03<Int16T, UseDelta, UseZigzag>(in, keys, data_S, data_M, data_L_low, data_L_high, count, prev);
}

template <typename Int16T, bool UseDelta, bool UseZigzag>
uint8_t const * decode_scalar_N03(
...
template <typename Int16T, bool UseDelta, bool UseZigzag>
size_t decode_N03(gsl::span<Int16T> out, gsl::span<uint8_t const> in, uint32_t dS, uint32_t dM, uint32_t dLl, Int16T prev = 0)
{   
    ...
    // }
#endif
    return decode_scalar_N03<Int16T, UseDelta, UseZigzag>(out, keys, data_S, data_M, data_L_low, data_L_high, prev) - in.begin();
}
...
arrow::Result<std::size_t> compress_signal_N03(   // COMPRESOR NANO_03 //
    ...
    bool is_last_batch)
{
    ...
    auto const res_sizes = svb16::encode_N03<int16_t, UseDelta, UseZigzag>(
        samples.data(), 
        keys->mutable_data(),
        data_S->mutable_data(),
        data_M->mutable_data(),
    ...
    unsigned long total_comp_size = sizeof(compressed_keys_size) + compressed_keys_size + sizeof(res_sizes.data_S_sz) + res_sizes.data_S_sz 
        + sizeof(compressed_data_M_size) + compressed_data_M_size + sizeof(res_sizes.data_Llow_sz) + res_sizes.data_Llow_sz + compressed_data_Lhigh_size;
    ...
    // Copy the size of uncompressed data_S
    std::memcpy(write_ptr, &res_sizes.data_S_sz, sizeof(res_sizes.data_S_sz));
    write_ptr += sizeof(res_sizes.data_S_sz);

    // Copy uncompressed data_S 
    std::memcpy(write_ptr, data_S->data(), res_sizes.data_S_sz);
    write_ptr += res_sizes.data_S_sz;
    ...
    // Copy the size of uncompressed data_L_low
    std::memcpy(write_ptr, &res_sizes.data_Llow_sz, sizeof(res_sizes.data_Llow_sz));
    write_ptr += sizeof(res_sizes.data_Llow_sz);
    
    // Copy uncompressed data_L_low
    std::memcpy(write_ptr, data_L_low->data(), res_sizes.data_Llow_sz);
    write_ptr += res_sizes.data_Llow_sz;
    ...
    comp_size_S += sizeof(res_sizes.data_S_sz) + res_sizes.data_S_sz;
    comp_size_M += sizeof(compressed_data_M_size) + compressed_data_M_size;
    comp_size_Llow += sizeof(res_sizes.data_Llow_sz) + res_sizes.data_Llow_sz;
    comp_size_Lhigh += compressed_data_Lhigh_size;

    return total_comp_size;
}

// DECOMPRESOR N03 //   ---->   FALTA AGREGAR LOS OTROS DECOMPRESORES 
pod5::Status decompress_signal_N03(
...
{   
    ...
    // Extract the size of UNCOMPRESSED data_S    
    std::size_t uncompressed_data_S_size;
    std::memcpy(&uncompressed_data_S_size, read_ptr, sizeof(uncompressed_data_S_size));
    read_ptr += sizeof(uncompressed_data_S_size) + uncompressed_data_S_size;
    ...
    if (ZSTD_isError(decompressed_zstd_data_M_size)) {
        return pod5::Status::Invalid(
            "M Input data not compressed by zstd: (",
    ...
    // Extract the size of UNCOMPRESSED data_Llow    
    std::size_t uncompressed_data_Llow_size;
    std::memcpy(&uncompressed_data_Llow_size, read_ptr, sizeof(uncompressed_data_Llow_size));
    read_ptr += sizeof(uncompressed_data_Llow_size) + uncompressed_data_Llow_size;


    // // Se calculan los tamaños de data Llow descomprimidos:
    // unsigned long long const decompressed_zstd_data_L_low_size =
    //     ZSTD_getFrameContentSize(read_ptr, uncompressed_data_Llow_size);
    // if (ZSTD_isError(decompressed_zstd_data_L_low_size)) {
    //     return pod5::Status::Invalid(
    //         "Llow Input data not compressed by zstd: (",
    //         decompressed_zstd_data_L_low_size,
    //         " ",
    //         ZSTD_getErrorName(decompressed_zstd_data_L_low_size),
    //         ")");
    // }
    // read_ptr += uncompressed_data_Llow_size;
    ...
    ARROW_ASSIGN_OR_RAISE(
        ...
        arrow::AllocateResizableBuffer( decompressed_zstd_keys_size +
                                        uncompressed_data_S_size +
                                        decompressed_zstd_data_M_size +
                                        uncompressed_data_Llow_size +
                                        decompressed_zstd_data_L_high_size +
    ...
    read_ptr += compressed_keys_size + sizeof(uncompressed_data_S_size);
    write_ptr += decompressed_zstd_keys_size;

    // Copy uncompressed data_S directly
    std::memcpy(write_ptr, read_ptr, uncompressed_data_S_size);
    read_ptr += uncompressed_data_S_size + sizeof(compressed_data_M_size);
    write_ptr += uncompressed_data_S_size;
    ...
    if (ZSTD_isError(decompress_res_data_M)) {
        return pod5::Status::Invalid(
            "M Input data failed to decompress using zstd: (",
            decompress_res_data_M,
            " ",
            ZSTD_getErrorName(decompress_res_data_M),
            ")");
    }
    read_ptr += compressed_data_M_size + sizeof(uncompressed_data_Llow_size);
    write_ptr += decompressed_zstd_data_M_size;  
    
    // Copy uncompressed data_Llow directly
    std::memcpy(write_ptr, read_ptr, uncompressed_data_Llow_size);
    read_ptr += uncompressed_data_Llow_size;
    write_ptr += uncompressed_data_Llow_size;

    // Decompress Data_Lhigh
    ...
    if (ZSTD_isError(decompress_res_data_L_high)) {
        return pod5::Status::Invalid(
            "Lhigh Input data failed to decompress using zstd: (",
    ...
    auto consumed_count = svb16::decode_N03<int16_t, UseDelta, UseZigzag>(destination, gsl::make_span(intermediate->data(), 
        intermediate->size()), uncompressed_data_S_size, decompress_res_data_M, uncompressed_data_Llow_size, 0);
