    ...
    Int16T prev = 0)
{
    ...
    for (uint32_t c = 0; c < count; c++) {
        ...
        // 2 bytes
            ...
            *data_L_low++ = static_cast<uint8_t>(value);
...
template <typename Int16T, bool UseDelta, bool UseZigzag>
buf_sizes_N02 encode_N02(Int16T const * in,
              uint8_t * SVB_RESTRICT keys,
              ...
              Int16T prev = 0)
{
    ...
    // }
#endif
    return encode_scalar_N02<Int16T, UseDelta, UseZigzag>(in, keys, data_S, data_M, data_L_low, data_L_high, count, prev);
}

template <typename Int16T, bool UseDelta, bool UseZigzag>
uint8_t const * decode_scalar_N02(
    ...
    Int16T prev = 0)
{
    ...
    for (uint32_t c = 0; c < count; c++) {
        ...
        if (code == 0) { // 0 bytes
            ...
            value = static_cast<uint8_t>(*data_M++);
        } else if (code == 3) { // 2 bytes
            value = (*data_L_high++ << 8) + *data_L_low++;
        } else {
...
template <typename Int16T, bool UseDelta, bool UseZigzag>
size_t decode_N02(gsl::span<Int16T> out, gsl::span<uint8_t const> in, uint32_t dS, uint32_t dM, uint32_t dLl, Int16T prev = 0)
{   
    ...
    // }
#endif
    return decode_scalar_N02<Int16T, UseDelta, UseZigzag>(out, keys, data_S, data_M, data_L_low, data_L_high, prev) - in.begin();
}
...
arrow::Result<std::size_t> compress_signal_N02(   // COMPRESOR NANO_01 //
    ...
    bool is_last_batch)
{
    ...
    auto const res_sizes = svb16::encode_N02<int16_t, UseDelta, UseZigzag>(
        samples.data(), 
        keys->mutable_data(),
        data_S->mutable_data(),
        data_M->mutable_data(),
    ...
    // TODO imprimir tamaño de cada buffer (y tasa de compression)
...
// DECOMPRESOR N02 //   ---->   FALTA AGREGAR LOS OTROS DECOMPRESORES 
pod5::Status decompress_signal_N02(
...
{   
    ...
    auto consumed_count = svb16::decode_N02<int16_t, UseDelta, UseZigzag>(destination, gsl::make_span(intermediate->data(), 
