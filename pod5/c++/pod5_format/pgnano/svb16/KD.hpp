...
//         uint16_t value = 0;
//         uint8_t code = (key_byte >> shift) & 0x01;
//         if (code == 1) {  // 2 bytes
//             std::memcpy(&value, data, sizeof(value));  // assumes little endian
//             data += 2;
//         } 
//         else {
//             value = (uint16_t)*data++;
    ...
    pgnano::PGNanoReaderState & state)
{
    ...
    // Se calculan los tamaños de data descomprimidos:
    ...
    // auto decompressed_size = decompressed_zstd_data_size + decompressed_zstd_keys_size;
