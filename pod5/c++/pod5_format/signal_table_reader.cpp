...
Status SignalTableReader::set_bam_file(const std::string & bam_file_path, const std::string & bam_index_file_path)
{
    m_pgnano_state->m_bam_handler->open_BAM_file(bam_file_path, bam_index_file_path); // FIXME: This operation can actually fail
    return Status::OK();
}

Status SignalTableReader::build_bam_idx()
{
    m_pgnano_state->m_bam_handler->build_query_index(); // FIXME: This operation can actually fail
    return Status::OK();
}

Status SignalTableReader::load_levels(const std::string & levels_path)
{
    m_pgnano_state->m_levels_table->parse_level_table(levels_path);// FIXME: This operation can actually fail
    return Status::OK();
}
