...
public:
    ...
    Status set_bam_file(const std::string & bam_file_path, const std::string & bam_index_file_path) override
    {
        return m_signal_table_reader.set_bam_file(bam_file_path, bam_index_file_path);
    }

    Status build_bam_idx() override
    {
        return m_signal_table_reader.build_bam_idx();
    }

    Status load_levels(const std::string & levels_path) override
    {
        return m_signal_table_reader.load_levels(levels_path);
    }
    ...
    FileReaderOptions const & options)
{
    ...
    ARROW_ASSIGN_OR_RAISE(auto read_table_reader, make_read_table_reader(reads_sub_file, pool));    // <----- ERROR!!!!!!
