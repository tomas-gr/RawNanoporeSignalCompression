...
/// \param[out]     compressed_signal_out       The compressed signal.
/// \param[inout]   compressed_signal_size      The number of compressed bytes, should be set to the size of compressed_signal_out on call.
POD5_FORMAT_EXPORT pod5_error_t pod5_pinanoraw_compress_signal(
    int16_t const * signal,
    size_t signal_size,
    char * compressed_signal_out,
    size_t * compressed_signal_size);
...
/*
/// This function should be called before writing POD5 files with PGNano. Next the caller must run pgnano_writer_build_BAM_idx
/// Executing this function more than once is UB
/// Multithreaded access is disallowed. Synchronization is client's responsibility 
...
/// This function should be called before writing POD5 files with PGNano.
/// This function must be called after pgnano_writer_set_BAM_file
/// Executing this function more than once is UB
/// Multithreaded access is disallowed. Synchronization is client's responsibility
/// After this function, the client is ready to write with PGNano compression, ONLY IF
/// The levels table has also been loaded
POD5_FORMAT_EXPORT pod5_error_t pgnano_writer_build_BAM_idx(Pod5FileWriter_t * writer);
/// This function should be called before reading POD5 files compressed with PGNano. Next the caller must run pgnano_reader_build_BAM_idx
/// Executing this function more than once is UB
/// Multithreaded access is disallowed. Synchronization is client's responsibility 
...
/// This function should be called before reading POD5 files with PGNano.
/// This function must be called after pgnano_reader_set_BAM_file
/// Executing this function more than once is UB
/// Multithreaded access is disallowed. Synchronization is client's responsibility
/// After this function, the client is ready to read with PGNano compression, ONLY IF
/// The levels table has also been loaded
POD5_FORMAT_EXPORT pod5_error_t pgnano_reader_build_BAM_idx(Pod5FileReader_t * reader);

/// This function should be called before writing POD5 files with PGNano. 
/// No synchronization with *_BAM_[build/set] is needed
/// Executing this function more than once is UB
/// Multithreaded access is disallowed. Synchronization is client's responsibility
...
/// Convenience function which groups setting and building the BAM index
/// As well as loading the levels table
/// Used to initialize writing
...
/// Convenience function which groups setting and building the BAM index
/// As well as loading the levels table
/// Initializes both reader and writer
