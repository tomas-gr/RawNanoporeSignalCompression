...
pod5_error_t pod5_pinanoraw_compress_signal(        // NUEVA FUNCIÓN
    int16_t const * signal,
    size_t signal_size,
    char * compressed_signal_out,
    size_t * compressed_signal_size)
{
    ...
    POD5_C_ASSIGN_OR_RAISE(
        auto buffer,
        pgnano::compress_signal(gsl::make_span(signal, signal_size), arrow::system_memory_pool(), read_data, false));
...
/*
POD5_FORMAT_EXPORT pod5_error_t pgnano_writer_set_BAM_file(const char * BAM_file, const char * index_file, Pod5FileWriter_t * writer)
{
    pod5_reset_error();
    if (!check_not_null(writer) || !check_not_null(BAM_file) || !check_not_null(index_file))
        return g_pod5_error_no;
    POD5_C_RETURN_NOT_OK(writer->writer->set_bam_file(BAM_file, index_file));
    return POD5_OK;
}

POD5_FORMAT_EXPORT pod5_error_t pgnano_writer_build_BAM_idx(Pod5FileWriter_t * writer)
{
    pod5_reset_error();
    if (!check_not_null (writer))
        return g_pod5_error_no;
    POD5_C_RETURN_NOT_OK(writer->writer->build_bam_idx());
    return POD5_OK;
}

POD5_FORMAT_EXPORT pod5_error_t pgnano_reader_set_BAM_file(const char * BAM_file, const char * index_file, Pod5FileReader_t * reader)
{
    pod5_reset_error();
    if (!check_not_null(reader) || !check_not_null(BAM_file) || !check_not_null(index_file))
        return g_pod5_error_no;
    POD5_C_RETURN_NOT_OK(reader->reader->set_bam_file(BAM_file, index_file));
    return POD5_OK;
}

POD5_FORMAT_EXPORT pod5_error_t pgnano_reader_build_BAM_idx(Pod5FileReader_t * reader)
{
    pod5_reset_error();
    if (!check_not_null(reader))
        return g_pod5_error_no;
    POD5_C_RETURN_NOT_OK(reader->reader->build_bam_idx());
    return POD5_OK;
}

POD5_FORMAT_EXPORT pod5_error_t pgnano_writer_load_levels(const char * levels_file, Pod5FileWriter_t * writer)
{
    pod5_reset_error();
    if (!check_not_null(levels_file) || !check_not_null(writer))
        return g_pod5_error_no;
    POD5_C_RETURN_NOT_OK(writer->writer->load_levels(levels_file));
    return POD5_OK;
}

POD5_FORMAT_EXPORT pod5_error_t pgnano_reader_load_levels(const char * levels_file, Pod5FileReader_t * reader)
{
    pod5_reset_error();
    if (!check_not_null(levels_file) || !check_not_null(reader))
        return g_pod5_error_no;
    POD5_C_RETURN_NOT_OK(reader->reader->load_levels(levels_file));
    return POD5_OK;
}

POD5_FORMAT_EXPORT pod5_error_t pgnano_writer_init_protocol(const char * BAM_file, const char * index_file, const char * levels_file, Pod5FileWriter_t * writer)
{
    pod5_error_t error = POD5_OK;
    error = pgnano_writer_set_BAM_file(BAM_file, index_file, writer);
    if (error != POD5_OK)
        return error;
    error = pgnano_writer_build_BAM_idx(writer);
    if (error != POD5_OK)
        return error;
    return pgnano_writer_load_levels(levels_file, writer);
}

POD5_FORMAT_EXPORT pod5_error_t pgnano_reader_init_protocol(const char * BAM_file, const char * index_file, const char * levels_file, Pod5FileReader_t * reader)
{
    pod5_error_t error = POD5_OK;
    error = pgnano_reader_set_BAM_file(BAM_file, index_file, reader);
    if (error != POD5_OK)
        return error;
    error = pgnano_reader_build_BAM_idx(reader);
    if (error != POD5_OK)
        return error;
    return pgnano_reader_load_levels(levels_file, reader);
}

POD5_FORMAT_EXPORT pod5_error_t pgnano_init_protocol(const char * BAM_file, const char * index_file, const char * levels_file, Pod5FileReader_t * reader, Pod5FileWriter_t * writer)
{
    pod5_error_t error = POD5_OK;
    error = pgnano_reader_init_protocol(BAM_file, index_file, levels_file, reader);
    if (error != POD5_OK)
        return error;
    return pgnano_writer_init_protocol(BAM_file, index_file, levels_file, writer);
}
*/
