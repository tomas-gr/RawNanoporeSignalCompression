...
// TODO: duplicated parameters: read_id and read_data => Can factor out to avoid overhead an make changes less impactful => This might affect the second add_signal method which I pressume is used for compressed signals
    ...
    pod5::Status set_bam_file(const std::string & bam_file_path, const std::string & bam_index_file_path)
    {
        if (!m_signal_table_writer || !m_read_table_writer)
        {
            return arrow::Status::Invalid("File writer closed, cannot set BAM file");
        }
        return m_signal_table_writer->set_bam_file(bam_file_path, bam_index_file_path);
    }

    pod5::Status build_bam_idx()
    {
        if (!m_signal_table_writer || !m_read_table_writer)
        {
            return arrow::Status::Invalid("File writer closed, cannot build custom BAM index");
        }
        return m_signal_table_writer->build_bam_idx();
    }

    pod5::Status load_levels(const std::string & levels_path)
    {
        if (!m_signal_table_writer || !m_read_table_writer)
        {
            return arrow::Status::Invalid("File writer closed, cannot read levels table");
        }
        return m_signal_table_writer->load_levels(levels_path);
    }
...
pod5::Status FileWriter::set_bam_file(const std::string & bam_file_path, const std::string & bam_index_file_path) { return m_impl->set_bam_file(bam_file_path, bam_index_file_path); }

pod5::Status FileWriter::build_bam_idx() { return m_impl->build_bam_idx(); }
pod5::Status FileWriter::load_levels(const std::string & levels_path) { return m_impl->load_levels(levels_path); }
